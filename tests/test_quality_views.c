/*-----------------------------------------------------------------------------
 * Umicom Studio IDE
 * File: tests/test_quality_views.c
 *
 * PURPOSE:
 *   Verify the test quality views behavior for
 *   Umicom Studio IDE.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#include <assert.h>
#include <string.h>

#include "quality_centre_fixture.h"
#include "umicom/studio/quality_views.h"

/* Verify existing view kinds retain stable IDs and primary counts. */
int main(void)
{
    UmiStudioQualityCentre *centre = umi_test_seeded_quality_centre();
    UmiStudioQualityView view;
    assert(umi_studio_quality_view_build(
        centre,
        UMI_STUDIO_QUALITY_VIEW_DUPLICATES,
        &view) == UMI_STATUS_OK);
    assert(strcmp(view.id, "duplicates") == 0);
    assert(view.item_count == 1U);
    assert(strstr(view.summary, "Duplicate Review") != NULL);
    umi_studio_quality_centre_destroy(centre);
    return 0;
}
