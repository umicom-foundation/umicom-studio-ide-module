 ...
 */
UmiStatus umi_studio_designer_project_create(const char *id,UmiStudioDesignerProject *out){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(out==NULL)return UMI_STATUS_INVALID_ARGUMENT;out->project=NULL;return umi_designer_project_create(id,&out->project);}void umi_studio_designer_project_dispose(UmiStudioDesignerProject *p){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(p!=NULL){umi_designer_project_destroy(p->project);p->project=NULL;}}
