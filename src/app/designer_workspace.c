/*-----------------------------------------------------------------------------
 * Umicom Studio IDE
 * File: applications/studio/src/app/designer_workspace.c
 *
 * PURPOSE:
 *   Implement Studio visual-designer workspace over Framework declarative and designer engines.
 *
...
UmiStatus umi_studio_designer_workspace_snapshot(UmiStudioServices *services,UmiStudioDesignerWorkspaceSnapshot *out)
{
    ...
    copy_text(out->view_type,sizeof(out->view_type),"studio.designer-workspace");
    copy_text(out->title,sizeof(out->title),"DesignerWorkspace");
    copy_text(out->summary,sizeof(out->summary),"Palette, hierarchy, properties, actions, signals, alignment, preview and templates.");
