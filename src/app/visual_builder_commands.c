/*-----------------------------------------------------------------------------
 * Umicom Studio IDE
 * File: src/app/visual_builder_commands.c
 *
 * PURPOSE:
 *   Implement the visual builder commands behavior for
 *   Umicom Studio IDE.
 *
 ...
 */
void umi_studio_visual_builder_command_init(UmiStudioVisualBuilderCommand *command,UmiStudioVisualBuilderCommandKind kind){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(command==NULL)return;(void)memset(command,0,sizeof(*command));command->kind=kind;}
/*
