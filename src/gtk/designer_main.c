 ...
 */
int main(int argc,char **argv){GtkApplication *app;AppState state={0};int result;/* Preserve the original failure result so the caller can respond to the correct cause. */ if(umi_studio_declarative_create(&state.declarative)!=UMI_STATUS_OK)return 1;/* Preserve the original failure result so the caller can respond to the correct cause. */ if(umi_studio_designer_create(state.declarative,&state.designer)!=UMI_STATUS_OK){umi_studio_declarative_destroy(state.declarative);return 1;}app=gtk_application_new("org.umicom.studio.designer",G_APPLICATION_DEFAULT_FLAGS);g_signal_connect(app,"activate",G_CALLBACK(activate),&state);result=g_application_run(G_APPLICATION(app),argc,argv);g_object_unref(app);umi_studio_designer_destroy(state.designer);umi_studio_declarative_destroy(state.declarative);return result;}
