/*-----------------------------------------------------------------------------
 * Umicom Studio IDE
 * File: applications/studio/runtime-manager/tests/test_runtime_presence.c
 *
 * PURPOSE:
...
int main(void){
    ...
    assert(umi_studio_runtime_manager_heartbeat(&m,"umicom.bank",1000U,500U)==UMI_STATUS_OK);
    assert(umi_studio_runtime_manager_is_present(&m,"umicom.bank",1200U));
    assert(!umi_studio_runtime_manager_is_present(&m,"umicom.bank",1600U));
    return 0;
}
