/*-----------------------------------------------------------------------------
 * Umicom Studio IDE
 * File: applications/studio/runtime-manager/tests/test_runtime_launch_plan.c
 *
 * PURPOSE:
...
int main(void){
    ...
    UmiStudioRuntimeLaunchPlan p;
    umi_studio_runtime_manager_init(&m);
    assert(umi_studio_runtime_catalogue_populate(&m)==UMI_STATUS_OK);
    assert(umi_studio_runtime_plan_launch(&m,"umicom.trader",&p)==UMI_STATUS_OK);
    assert(p.readiness==UMI_STUDIO_RUNTIME_LAUNCH_READY);
    assert(p.can_launch);
    assert(umi_studio_runtime_plan_launch(&m,"umicom.studio-ide",&p)==UMI_STATUS_OK);
    assert(p.readiness==UMI_STUDIO_RUNTIME_LAUNCH_ALREADY_RUNNING);
    return 0;
}
